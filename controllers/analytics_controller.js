...
exports.analytics = async (req, res, next) => {
  ...
  try {
    ...
    const bestTime = {
      'value': `${finished.map((t) => t.retos.map((r) => Math.round((r.retosSuperados.createdAt - t.turno.startTime) / 600) / 100).reduce((a, b) => a > b ? a : b, Math.Infinity)).reduce((a, b) => a < b ? a : b, Math.Infinity) || 0} min.`,
      'icon': 'timer'
    }
    const sucessRate = {
    ...
    console.dir(teams[0].requestedHints.map(h => h.hint ? h.hint.id : 0), {depth: 10})
    ...
    res.render('escapeRooms/analytics/analytics', {
      escapeRoom,
      turnId,
      summary,
      charts
...
exports.puzzlesByParticipants = async (req, res, next) => {
  ...
  try {
    ...
    if (!csv) {
      res.render('escapeRooms/analytics/retosSuperadosByParticipant', {
        escapeRoom,
        results,
        turnId,
        orderBy
...
exports.puzzlesByTeams = async (req, res, next) => {
  ...
  try {
    ...
    if (!csv) {
      res.render('escapeRooms/analytics/retosSuperadosByTeam', {
        escapeRoom,
        results,
        turnId
      })
...
exports.ranking = async (req, res, next) => {
  ...
  try {
    ...
    const teams = result.map((teamRes) => ({
      ...teamRes.dataValues,
...
exports.progress = async (req, res, next) => {
  ...
  try {
    ...
    res.render('escapeRooms/analytics/progress', {
...
exports.histogram = async (req, res, next) => {
  ...
  try {
    const teams = await models.team.findAll(queries.team.teamComplete(escapeRoom.id, turnId)).map((team) => {
      const {turno} = team

      return {
        'id': team.id,
        'retosSuperados': team.retos.map((reto) => ({
          'id': reto.id,
    ...
    res.render('escapeRooms/analytics/histogram', {
      'escapeRoom': req.escapeRoom,
      'puzzles': result
    })
  } catch (e) {
...
exports.grading = async (req, res, next) => {
  ...
  try {
    ...
    if (csv) {
      ...
      res.render('escapeRooms/analytics/grading', {
        escapeRoom,
        results,
        turnId,
        orderBy
